...
|--------|------|-------------|
| POST | `/webhook` | Falco Sidekick 이벤트 수신 |
| GET | `/healthz` | Health check |
| GET | `/readyz` | Readiness check |
| GET | `/metrics` | Prometheus 메트릭 |
| GET | `/api/v1/events` | 이벤트 목록 (`?severity=`, `?namespace=`, `?limit=`) |
| GET | `/api/v1/events/summary` | 대시보드용 집계 (severity/rule/namespace/action별) |
| GET | `/api/v1/events/:id` | 단일 이벤트 상세 |
| GET | `/api/v1/isolations` | 활성 격리 NetworkPolicy 목록 (`?namespace=` 필터 지원) |
| DELETE | `/api/v1/isolations/:ns/:name` | 격리 해제 |
| POST | `/api/v1/heartbeat` | Watchdog 생존 신호 수신 (falco_status 포함) |
| GET | `/api/v1/falco/status` | Falco heartbeat / 침묵 탐지 상태 조회 (healthy 시 200, silenced 시 503) |
