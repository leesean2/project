...
class MetricsStore:
    ...
    def __init__(self):
        ...
        # HMAC 강제 모드 ("soft" | "hard")
        self.hmac_mode = "soft"

        # Webhook 보안 거부 카운터 {status_code: count}
        # 401=HMAC실패, 403=IP차단, 413=바디초과, 429=속도초과
        self.webhook_rejected = {}          # {status_code: count}

        # 정규화된 사유별 거부 카운터 (Prometheus 레이블용)
        # reason: hmac_failed | ip_blocked | rate_limited | body_too_large | other
        self.webhook_rejected_by_reason = {}  # {reason: count}
    ...
    def set_hmac_mode(self, hard: bool):
        """서버 시작 시 HMAC 강제 모드를 기록 — 대시보드에서 즉시 확인 가능."""
        with self._lock:
            self.hmac_mode = "hard" if hard else "soft"

    def inc_webhook_rejected(self, status_code: int, reason: str = ""):
        """웹훅 거부 카운터를 증가시킨다. status_code와 정규화된 사유를 모두 추적."""
        with self._lock:
            key = str(status_code)
            self.webhook_rejected[key] = self.webhook_rejected.get(key, 0) + 1

            # HTTP 상태코드 → 짧은 레이블로 정규화 (고카디널리티 방지)
            normalized = {
                "401": "hmac_failed",
                "403": "ip_blocked",
                "413": "body_too_large",
            ...
            self.webhook_rejected_by_reason[normalized] = (
                self.webhook_rejected_by_reason.get(normalized, 0) + 1
            )
    ...
    def render(self) -> str:
        ...
        with self._lock:
            ...
            # --- HMAC 강제 모드 ---
            lines.append("# HELP compliance_hmac_mode HMAC 강제 모드 (1=hard/필수, 0=soft/선택)")
            lines.append("# TYPE compliance_hmac_mode gauge")
            lines.append(
                f'compliance_hmac_mode{{mode="{self.hmac_mode}"}} '
                f'{1 if self.hmac_mode == "hard" else 0}'
            )

            # --- Webhook 사유별 거부 통계 ---
            lines.append("# HELP compliance_webhook_rejected_by_reason_total 보안 사유별 거부된 웹훅 요청 수")
            lines.append("# TYPE compliance_webhook_rejected_by_reason_total counter")
            for reason_key, count in self.webhook_rejected_by_reason.items():
                safe = reason_key.replace('"', '\\"')
                lines.append(
                    f'compliance_webhook_rejected_by_reason_total{{reason="{safe}"}} {count}'
                )
